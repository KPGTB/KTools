...
public class BarParser implements IParamParser<KBar> {
    @Override
    public KBar convert(String param, ToolsObjectWrapper wrapper) {
        return wrapper.getBarManager().getBar(param);
    }

    @Override
    public boolean canConvert(String param, ToolsObjectWrapper wrapper) {
        return wrapper.getBarManager().getBars().containsKey(param);
    }

    @Override
    public @NotNull List<String> complete(String arg, CommandSender sender, ToolsObjectWrapper wrapper) {
        return wrapper.getBarManager().getBars().keySet().stream()
                .filter(bar -> bar.startsWith(arg))
                .limit(30)
